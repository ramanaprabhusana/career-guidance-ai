...
| F-C (extends AN-004, AN-013) | P0 | Filler Guard | All phases | Acknowledgement tokens that commonly appear after non-question turns must be caught by filler-guard as a deterministic backstop — unconditionally, regardless of LLM intent | User says `sure`, `understood`, `got it`, `alright`, `I see`, `makes sense`, `right`, `fair enough`, `sure thing` | Filler guard fires unconditionally; analyzerOutput stripped of durable writes even if LLM says "confirm" | "sure" slips through because LLM classifies it as "confirm" after a bridge turn, and `user_intent !== "confirm"` guard blocks the text match | `src/nodes/filler-guard.ts` — separate `UNCONDITIONAL_FILLER_PATTERNS` list; two-tier isFiller logic: unconditional patterns bypass the confirm override; context-sensitive patterns (ok, fine) retain it | TC-014: "sure" after non-question turn → filler guard fires regardless of user_intent value | Implemented | Two-tier fix: unconditional backstop for bridge acks; confirm override preserved for ok/fine which legitimately confirm yes/no questions. |
...
| File | Change | Rule(s) |
|------|--------|---------|
| `agent_config/skills/exploration_role_targeting/speaker.md` | FORBIDDEN BRIDGE PHRASES section added before Opening Message; MANDATORY FIRST MESSAGE rule: first turn must contain role confirmation + Skill 1 in same message | SP-012 |
| `agent_config/prompts/analyzer_template.md` | CRITICAL caveat added to confirm rule: only classify as "confirm" when prior assistant message ended with yes/no question or choice prompt; bridge/transition turns → "filler" | AN-005 |
| `src/nodes/filler-guard.ts` | Two-tier filler logic: new `UNCONDITIONAL_FILLER_PATTERNS` list fires regardless of LLM user_intent (sure, got it, understood, alright, I see, makes sense, right, fair enough, sure thing); existing `FILLER_PATTERNS` retain confirm override for context-sensitive tokens (ok, fine). `isFillerOrAmbiguous` checks both lists. `fillerGuard` uses `isUnconditionalFiller \|\| isContextualFiller \|\| intentIsFiller`. | F-C (AN-004, AN-013) — backstop now truly unconditional |
